...
set key [get_line_from_file "../Node0/Diffie-Hellman/flow.dat" 7]
...
proc AES_Decryption {fIn key} {
	...
	set fOut [open "../Node0/Messages/Operations/aes_EDFlow_Helper.txt" w]
...
# Overview: flag.txt file contains a 2 bits string, it is initiated to differentiate netween 
# the 4 phases og this application.
# Methodology: open the flag.txt file, read the content, close the file, the four phases are 
# described in the if,elseif code block!
set flag_handler [open "./assets/flag.txt" r]
set flag [read $flag_handler]
close $flag_handler

if {$flag == "00"} {
	set A [get_line_from_file "../Node0/Diffie-Hellman/flow.dat" 4]
	set B [get_line_from_file "../Node0/Diffie-Hellman/flow.dat" 5]

	set_line_in_file "../Node0/Messages/Outgoing/aes.dat" 1 $B
	set_line_in_file "../Node0/Messages/Outgoing/aes.dat" 2 $A
	
	set plaintext [open "../Node0/Messages/Outgoing/aes.dat" r]
	AES_Encryption $plaintext $key $iv
	
	set rsa_file "../Node0/Messages/Operations/rsa_EDFlow.txt"
	set aes_file "../Node0/Messages/Operations/aes_EDFlow.txt"
	set destination "../Node0/Messages/Outgoing/outgoing.txt"
	cuncatenate_messages $rsa_file $aes_file $destination
} elseif {$flag == "11"} {
	set message [get_line_from_file "../Node0/Messages/Incoming/incoming.txt" 3]
	
	set fOut [open "../Node0/Messages/Operations/aes_EDFlow.txt" w]
	puts $fOut $message
	close $fOut
	
	set fIn [open "../Node0/Messages/Operations/aes_EDFlow.txt" r]
	AES_Decryption $fIn $key
	
	set rsa "../Node0/Messages/Operations/fito.txt"
	set aes "../Node0/Messages/Operations/aes_EDFlow.txt"
	set destination "../Node0/Messages/Operations/decryption.txt"
