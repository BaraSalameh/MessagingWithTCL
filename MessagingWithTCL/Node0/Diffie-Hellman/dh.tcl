...
if {$flag eq "0"} {
	set p [get_line_from_file "../Node0/Diffie-Hellman/flow.dat" 1]
	set g [get_line_from_file "../Node0/Diffie-Hellman/flow.dat" 2]
	...
	set_line_in_file "../Node0/Diffie-Hellman/flow.dat" 3 $a
	set_line_in_file "../Node0/Diffie-Hellman/flow.dat" 4 $A
	set_line_in_file "./Diffie-Hellman/flow.dat" 1 $A
} elseif {$flag eq "1"} {
	...
	set_line_in_file "../Node0/Diffie-Hellman/flow.dat" 6 $s
	
	set hex [format "%x" $s]
	set length [string length $hex]
	if {$length eq 16 || $length eq 24 || $length eq 32} {
		set_line_in_file "../Node0/Diffie-Hellman/flow.dat" 7 $hex
		puts "Node1{hex}: $hex"
	} else {
		puts "The key length is too short or too long, re-run the application. exiting..."
		exit
	}
}
